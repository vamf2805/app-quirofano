...
class PDF extends FPDF{
	...
	$this->Cell(40);
	// Título
	$this->Cell(180, 10, 'Reporte por fecha',0,0,'C');
	// Salto de línea
	$this->Ln(20);
	}
	...
	$fechaF = $_GET['fechaF'];
	$mysqli=conectar();
	$query     = ("SELECT * FROM `horarios` WHERE `fecha_horarios`>='$fecha' AND `fecha_horarios`<='$fechaF' AND `estado`='ocupado' ORDER BY fecha_horarios DESC");
...
$pdf = new PDF('L');
$header = array('Fecha',utf8_decode('Quirófano'),utf8_decode('Médico'),'Paciente','Horario', utf8_decode('Intervención'));
$pdf->AddPage();
