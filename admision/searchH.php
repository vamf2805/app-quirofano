...
if($_SESSION['logueado'] != "SI" and $_SESSION['admin']!=1 AND $_SESSION['admin']!=2){
    ...
    if($turno == 'Mañana'){
         $query = ("SELECT id_horario,hora,estado,fecha_horarios, quirofano FROM `horarios` WHERE `fecha_horarios`='$fecha' AND `hora` >= '07:00:00' AND `hora` <='12:00:00'");
        ...
        $html->display("searchH.html");
    } else {
        $alerta ="<script>alertify.alert('Mensaje de alerta','No existen horarios para la fecha: $fecha', function (e) {
        if (e) {
             window.location.href='hourslist.php?fecha=$fecha&turno=$turno';
            return true;
            }
        }).set({transition:'fade'}).set('closable', false).show();</script>";
        $html->assign("turno", $turno);
        $html->assign("alerta", $alerta);
        $html->display("hourslist.html");
        }
    }

    if($turno == 'Tarde'){
         $query     = ("SELECT id_horario,hora,estado,fecha_horarios, quirofano FROM `horarios` WHERE `fecha_horarios`='$fecha' AND `hora` >= '13:00:00' AND `hora` <='18:00:00'");
         $resultado = $mysqli->query($query);
         if ($mysqli->affected_rows > 0) {
        while ($fila = $resultado->fetch_array()) {
            //Mostrar los valores para la tabla, valor unico cedula del paciente
